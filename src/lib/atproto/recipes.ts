...
export async function resolveHandle(handle: string): Promise<string> {
  const data = await xrpcGet<{ did: string }>("https://public.api.bsky.app", "com.atproto.identity.resolveHandle", { handle });
  return data.did;
}
...
export async function fetchRecipe(ref: string): Promise<RecipeResult> {
  ...
  const data = await xrpcGet<{ uri: string; cid?: string; value: RecipeRecord }>(pds, "com.atproto.repo.getRecord", { repo: did, collection, rkey });
