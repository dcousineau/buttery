...
const isLoopback =
  ['127.0.0.1', 'localhost', '[::1]'].includes(rawHostname) ||
...
export const APP_URL = isLoopback
...
export const clientMetadata: OAuthClientMetadataInput = isLoopback
  ? atprotoLoopbackClientMetadata(
      `http://localhost?redirect_uri=${encodeURIComponent(REDIRECT_URI)}&scope=${encodeURIComponent('atproto')}`,
    )
  : {
...
function createStateStore(): NodeSavedStateStore {
  ...
  return {
    async set(key, state) {
      ...
      await pool.query(
        `delete from "atproto_oauth_state" where "createdAt" < now() - interval '1 hour'`,
      )
    },
    async get(key) {
      const res = await pool.query<{ value: string }>(
        `select "value" from "atproto_oauth_state" where "key" = $1`,
        [key],
      )
    ...
    async del(key) {
      await pool.query(`delete from "atproto_oauth_state" where "key" = $1`, [
        key,
      ])
    },
  }
...
function createSessionStore(): NodeSavedSessionStore {
  ...
  return {
    ...
    async get(did) {
      const res = await pool.query<{ value: string }>(
        `select "value" from "atproto_oauth_session" where "key" = $1`,
        [did],
      )
      const row = res.rows[0]
    ...
    async del(did) {
      await pool.query(
        `delete from "atproto_oauth_session" where "key" = $1`,
        [did],
      )
    },
  }
