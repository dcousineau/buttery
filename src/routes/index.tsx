...
export const Route = createFileRoute('/')({
  validateSearch: (search: Record<string, unknown>): { auth_error?: string } =>
    typeof search.auth_error === 'string'
      ? { auth_error: search.auth_error }
      : {},
  component: App,
})
    ...
    <main className="page-wrap px-4 pb-8 pt-14">
      <section className="island-shell rise-in rounded-[2rem] px-6 py-10 sm:px-10 sm:py-12">
        ...
        <p className="m-0 max-w-2xl text-base text-[var(--sea-ink-soft)]">
          Sign in with your atproto account and look up recipes stored in the{' '}
          <code>exchange.recipe.recipe</code> lexicon.
        </p>
      </section>
      ...
      {meta.length > 0 && (
        <ul className="mb-4 flex flex-wrap gap-2 pl-0">
          {meta.map((m) => (
            <li
              key={m}
              className="list-none rounded-full border border-[var(--chip-line)] bg-[var(--chip-bg)] px-3 py-1 text-xs text-[var(--sea-ink)]"
            >
              {m}
            </li>
          ))}
...
function formatDuration(iso: string): string {
  ...
  const [, h, m, s] = match
  return (
    [h && `${h}h`, m && `${m}m`, s && `${s}s`].filter(Boolean).join(' ') || iso
  )
}
