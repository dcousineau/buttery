...
export function AddRecipesSheet({
  ...
  async function onAdd() {
    ...
    setFailed(false);
    try {
      const result = await file.mutateAsync({ collectionId: collection.id, recipeIds: picked });
      ...
      if (!result.ok) {
        setBlocked(result.reason === "recipes_unpublished" ? result.recipeIds : picked);
        return;
      }
      close();
    } catch {
      ...
      <SheetContent side="bottom" showCloseButton={false} className="max-h-[88svh] gap-0 p-0 data-[side=bottom]:rounded-t-xl">
        ...
        <SheetFooter className="flex-none border-t-2 border-border">
          {blockedTitles.length > 0 && (
            // TODO(m5): this is where "Publish recipe & add" goes — the same
            // combo the picker's blocked row carries, over a whole selection.
            <p className="m-0 rounded-lg border-2 border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-foreground" role="status">
              {collection.name} is published, so it can’t hold a private recipe yet: {blockedTitles.join(", ")}. Publish {blockedTitles.length === 1 ? "it" : "them"} first, then
              add.
            </p>
          )}
          {failed && (
            <p className="m-0 rounded-lg border-2 border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-foreground" role="status">
              That didn’t save. Nothing was filed — try again.
            </p>
          )}
          <Button
            size="lg"
