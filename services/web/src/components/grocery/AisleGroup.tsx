  ...
  return (
    <section aria-labelledby={headingId} data-aisle={aisle} className="flex flex-col">
      <h2
        ...
        // `md` the window is the scrollport, so however much of the fixed app
        // header is showing (`--header-offset`) counts too.
        className="sticky top-[calc(var(--header-offset)+var(--pane-pinned-height,0px))] z-10 flex md:top-[var(--pane-pinned-height,0px)] items-baseline gap-2 bg-background px-3 pt-3 pb-1.5 text-xs font-bold tracking-[0.08em] text-muted-foreground uppercase md:px-4"
      >
        {label}
