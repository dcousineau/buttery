 ...
 * costs a full atproto sweep to rebuild and has nothing to do with the
 ...
 * ── WHAT IS NOT HANDLED ─────────────────────────────────────────────────────
 *
 * Blob storage. Hero images for deleted local recipes (`recipe_image.blob_cid`,
 * `recipe_pending_image.object_key`) are orphaned in the bucket, not deleted —
 * this script only speaks SQL. In local dev that is a few stray objects; nobody
 * pays for them and nothing reads them.
 *
