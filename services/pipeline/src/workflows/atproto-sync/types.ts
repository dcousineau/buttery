 ...
 * workflow and the per-repo workflow are separate queues, often on separate
 * worker replicas, and everything crossing between them is JSON in Redis. These
 * types are that wire format — keep them small, keep them JSON, and remember
 * that a job enqueued by one deployment may be picked up by the next.
 */
...
/** The batch workflow's return value, and the shape of the `atproto_sync_run` row. */
