...
const enumerate: StepSpec = {
  ...
  run: async ({ payload, log: line, flow, redis }) => {
    const scope = scopeOf(payload);
    const config = loadSyncConfig(scope);

    const lock = await acquireLock(redis, LOCK_KEY, LOCK_TTL_MS);
    if (!lock) {
      ...
      log.warn("sweep skipped — another sweep holds the lock");
    ...
    try {
      ...
      const syncRunId = await openSyncRun(pool, config);
      ...
      const pool = getPool(config.databaseUrl);
      await closeSyncRun(pool, null, emptySummary(config.dryRun), String(err));
      await releaseLock(redis, LOCK_KEY, lock);
      throw err;
    }
  },
};
...
const syncRepo: StepSpec = {
  ...
  run: async ({ payload, log: line, enqueue }) => {
    ...
    try {
      ...
      log.error("repo sweep failed", { did, permanent, err: String(err) });
      if (!config.dryRun) await markRepoError(pool, did, String(err));
...
const finalize: StepSpec = {
  ...
  run: async ({ payload, children, log: line, redis }) => {
    const input = payload as FinalizePayload;
    const config = loadSyncConfig(input);
    const pool = getPool(config.databaseUrl);

    const results = await children();
    let summary = foldRepos({ ...emptySummary(config.dryRun), syncRunId: input.syncRunId, reposSeen: input.dids.length }, results.values as RepoOutcome[], results.failures.length);

    try {
      if (input.fullSweep && !config.dryRun) {
        summary = { ...summary, reposMarkedMissing: await markMissingRepos(pool, input.dids) };
        log.info("marked missing repos", { count: summary.reposMarkedMissing });
      }
      await closeSyncRun(pool, input.syncRunId, summary, null);
      ...
      await closeSyncRun(pool, input.syncRunId, { ...summary, status: "error" }, String(err));
      throw err;
    } finally {
      ...
      await releaseLock(redis, LOCK_KEY, input.lock);
    }
  },
};

export const steps: readonly StepSpec[] = [enumerate, syncRepo, finalize];
