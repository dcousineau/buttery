...
import { isLlmEnrichmentEnabled, shutdown as shutdownPosthog } from "#/workflows/recipe-enrichment/lib/posthog.ts";
import { resolveProvider } from "#/workflows/recipe-enrichment/lib/provider.ts";
import { fetchPrompt } from "#/workflows/recipe-enrichment/lib/prompt-fetch.ts";
...
import { buildMessages, buildRecipeJson, modelRawText } from "#/workflows/recipe-enrichment/lib/llm-messages.ts";

/**
 * Derive per-recipe allergen/diet labels, then ask a model for a second
 * opinion. Two steps: `enrich` (rules, entry) and `llm-enrich` (LLM second
 * opinion). The two providers write disjoint rows in one
 * `recipe_enrichment_label` table, split by the `method` column's prefix
 * (`rules@N` vs `llm:%`).
 */
  ...
  run: async ({ payload, log: line, enqueue }) => {
    ...
    try {
      ...
      try {
        ...
        log.warn("failed to enqueue llm-enrich", { recipeId, err: err instanceof Error ? err.message : String(err) });
      }
