...
```
pnpm -r test                     → 404 passed | 102 skipped   (34 files)
pnpm --filter @buttery/web test:db                    → 97 passed  (4 files)
pnpm --filter @buttery/atproto-cron-sync test:db      →  4 passed  (1 file)
pnpm -r typecheck                → clean
pnpm lint                        → clean
```

The 102 skipped in `pnpm -r test` are the DB suites skipping themselves for want of
`DATABASE_URL`; they are the same 101 that pass under the two `test:db` commands.
...
8. **No virtualization library.** `package.json` has none and §14/D3 forbid adding one, so
   the review list uses `content-visibility` with a measured `contain-intrinsic-size`.
   Honest caveat: a reflow micro-benchmark at 340 rows did **not** show a win. The real wins
   from that investigation were finding the wrong scroll container and the wrong intrinsic
   size, both fixed. Scrolling 317 rows is smooth in practice.
