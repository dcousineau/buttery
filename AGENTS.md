...
- Sync target network = `services/atproto-cron-sync/.env`, nothing else. Default live atmosphere; local dev-env need `SYNC_PDS_URL=http://localhost:2583` + `ATPROTO_PLC_URL=http://localhost:2582` in that file (dev-env ship no relay; its PDS reject unauthenticated `listReposByCollection`). Same file whoever drive sweep. Sweep idempotent — re-run after each publish. Three way run it: `pnpm --filter @buttery/atproto-cron-sync sync:once`, process-compose `atproto-cron-sync` (disabled one-shot, start by hand), or through queue: `curl -X POST http://127.0.0.1:3002/jobs/atproto-sync -d '{}' -H 'content-type: application/json'`.
- No Railway cron service any more. Schedule live in BullMQ as `atproto-sync` pipeline (`services/pipeline`), reconciled at server boot from `ATPROTO_SYNC_SCHEDULE`. Blank locally on purpose — laptop no sweep live atmosphere in background.
- New background job = new file in `services/pipeline/src/jobs/` + one entry in `PIPELINES`. Server, worker and autoscaler all read that one registry. Always set `defaultJobOptions.removeOnComplete`/`removeOnFail` — BullMQ keep finished jobs forever otherwise.
- `services/pipeline/tsconfig.json` deliberately have NO `paths` for `#/*`. It typecheck `@buttery/atproto-cron-sync` source too, and a `paths` entry no scoped to one package — it capture that package's `#/…` imports and resolve them wrong. `imports` in each package.json already enough.
