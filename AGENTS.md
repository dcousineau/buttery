...
- New background job = new FOLDER in `services/pipeline/src/workflows/` + one entry in `WORKFLOWS`. Server, worker, autoscaler and `run:once` CLI all read that one registry. Always set `defaultJobOptions.removeOnComplete`/`removeOnFail` — BullMQ keep finished jobs forever otherwise.
- Workflow = queue + ordered `steps`, via `defineWorkflow` (`src/workflows/define.ts`). Kernel own progress, per-step job log, failing-step name, cursor. `start(payload)` = only place workflow parse `job.data`. Never return `state` from `result` — it hold config, config hold connection string.
- `resumeOnRetry` OFF by default and stay off unless every step work from `start()` state + what earlier steps wrote DURABLY. `state` rebuilt each attempt, no carried across. `demo` = on (reference), `atproto-sync` = off (need in-memory DID list).
- Long workflow that also have schedule need `exclusive: {key, ttlMs}` — BullMQ stop same job twice, no stop two different job on one queue. Loser SKIP (`{status:"skipped"}`), no fail. Workflow that open durable row need `onFailure` too, else dead run leave row say `running` forever.
