...
- DB connection = `DATABASE_URL` in per-service `.env` (`services/web/.env`, `services/worker/.env`), fixed port from repo-root `docker-compose.yml` (55432). Each service load own file — `kysely.config.ts`, `vite.config.ts`, worker `src/env.ts` + `vitest.config.ts` — no wrapper. `scripts/dev/bootstrap-env.mjs` create missing ones from `.env.example`. Only web `pnpm test:db` still wrap `railway run --service buttery --`.
- Sweep target network = `services/worker/.env`, nothing else. Default live atmosphere; local dev-env need `SYNC_PDS_URL=http://localhost:2583` + `ATPROTO_PLC_URL=http://localhost:2582` in that file (dev-env ship no relay; its PDS reject unauthenticated `listReposByCollection`). Start sweep = process-compose `atproto-sync` (disabled one-shot), or `temporal workflow execute --namespace buttery --task-queue buttery --type atprotoSync --workflow-id atproto-sync --input '{}'`. Per-run knob (`dryRun`, `maxRepos`, `onlyDid`, `parallelism`) go in `--input`, never env. `worker` process must be up — CLI only start workflow, worker run it. Sweep idempotent — re-run after each publish.
- Temporal workflow code (`services/worker/src/workflows/*/workflow.ts` + what it import) run in deterministic sandbox: no `pg`, no `node:*`, no `process`, no clock. Put IO in `activities.ts`, pure helper in `plan.ts`, node-only work in `lib/`. `src/workflows.test.ts` build the bundle and fail when someone reach into `lib/` from workflow. Export name in `src/workflows.ts` IS workflow type — mismatch = workflow that start then fail every task.
- Everything live in `buttery` namespace, never `default` — schedule reconcile delete anything in namespace repo no declare. Activities take dependency (pg pool) from `worker.ts`, no module singleton.
