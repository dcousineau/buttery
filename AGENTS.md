...
- Sweep target network = `services/worker/.env`, nothing else. Default live atmosphere; local dev-env need `SYNC_PDS_URL=http://localhost:2583` + `ATPROTO_PLC_URL=http://localhost:2582` in that file (dev-env ship no relay; its PDS reject unauthenticated `listReposByCollection`). Same either way: `pnpm --filter @buttery/worker sync:once`, or process-compose `atproto-sync` (disabled one-shot, start by hand). Both start the workflow and wait — `worker` process must be up. Sweep idempotent — re-run after each publish.
- Temporal workflow code (`services/worker/src/workflows/*/workflow.ts` + what it import) run in deterministic sandbox: no `pg`, no `node:*`, no `process`, no clock. Put IO in `activities.ts`, pure helper in `plan.ts`, node-only work in `lib/`. `bundle.test.ts` build the bundle and fail when someone reach into `lib/` from workflow. Export name in `workflows/bundle.ts` IS workflow type — mismatch = workflow that start then fail every task.
