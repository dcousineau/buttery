...
- Sync target network = `services/pipeline/.env`, nothing else. Default live atmosphere; local dev-env need `SYNC_PDS_URL=http://localhost:2583` + `ATPROTO_PLC_URL=http://localhost:2582` in that file (dev-env ship no relay; its PDS reject unauthenticated `listReposByCollection`). Same file whoever drive sweep. Sweep idempotent — re-run after each publish. Three way run it: `pnpm --filter @buttery/pipeline sync:trigger`, process-compose `atproto-sync` (disabled one-shot, start by hand), or through queue: `curl -X POST http://127.0.0.1:3002/jobs/atproto-sync -d '{}' -H 'content-type: application/json'`. All three take same fleet-wide Redis lock — second one SKIP, no fail.
